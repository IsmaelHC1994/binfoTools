# ismaelhc94@gmail.com
# VCF annotation from GTF 

import re, gzip, argparse, pickle
from os import listdir
from bisect import bisect_right, bisect_left
from multiprocessing import Process

# The following script annotates to a genetic variants/mutation file the genes affected by these variants, according to genetic location. 

# We configure a subclass of multiprocessing.Process to parallelize the vcf files so each core will handle several vcf files at the same time, if needed.
...
class annotation_process(Process):
    ...
    def __init__(self: list, batch: list, gtf_dict: dict, window_limit: int, output: str):
    ...
    def run(self):
        ...
        for i in self.batch:
            ...
            output_name = (i.split('/')[-1]).replace('.gz', '_ann.gz')
            # output_name = self.vcf_files[i].replace('.gz', '_ann.gz')
            
            file_out = gzip.open(f'{self.output}/{output_name}', 'wt')
...
def annotation_parallel(vcf_files: list, num_process: int, window_limit: int, gtf_dict: dict, output: str):
    ...
    for i in range(0, num_process):
        ...
        process_list.append(annotation_process(batch[i], gtf_dict, window_limit, output))
...
if __name__ == '__main__':
    ...
    pars = argparse.ArgumentParser(description= 'VCF-GTF annotation')
    pars.add_argument("-VCF", 
                    help="Directory with all the VCF.gz files",
                    required=True)
    pars.add_argument("-gtf", 
                      help='pickle dictionary file with GTF information ', required=True)
    pars.add_argument("-outputDir",
    ...
    # Read pickle-GTF file
    with open(args.gtf, 'rb') as gtf_file:
        gtf_dict = pickle.load(gtf_file)
    
    annotation_parallel(vcf_files, int(args.num_process), int(args.pb), gtf_dict, args.outputDir)