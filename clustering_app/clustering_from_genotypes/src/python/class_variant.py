...
class Variant:
    ...
    def genotype_samples_variant(self, numerical='False'):
        """
        ...
        for i in range(len(self.genotypes)):
            ...
            if self.__check_if_homozygous_for_mut(haplotype):
                dict_values[index] = values[0]
            elif self.__check_if_heterozygous_for_mut(haplotype):
                dict_values[index] = values[1]
            elif self.__check_if_reference(haplotype):
                dict_values[index] = values[2]
    ...
    def count_presence_per_group(self, groups:dict):
        """_summary_
        ...
        for i in range(len(self.genotypes)):
            ...
            haplotype = self.sample_data[sample]["GT"]
            
            if self.__check_if_homozygous_for_mut(haplotype) or self.__check_if_heterozygous_for_mut(haplotype):    
        
                for k in groups:
                    if sample in groups[k]:
                        results[int(k) - 1] += 1
    ...
    def return_id_variant(self, sep):
        ...
        return sep.join(
            # [chr.replace("chr", ""), pos, self.ref, self.return_alt(ignore_indels=True)] leaves only chr number
            [chr, pos, self.ref, self.return_alt(ignore_indels=True)]
        )
